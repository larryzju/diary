...
bus connects CPU and DRAM
- read transaction: transfers data from the main memory to the CPU
- write transaction: transfers data from the CPU to the main memory

there are three types of bus
1. address bus
2. data bus
3. control bus (can share the data bus): carry signals to sync the transition and identify what kind of transaction is
   1. to main memory, or to some other I/O device, such as a disk controller
   2. read or write
   3. address or data item?

[[./img/bus-between-cpu-and-memory.png]]

As shown in above figure
- system bus connects the CPU to the I/O bridge
- memory bus connects the I/O bridge to the main memory
- I/O bridge includes the memory controller, tranlate the electrical signals
- I/O bridge also connects with other I/O devices such as disks and graphics cards

#+BEGIN_VERSE
For Intel system, CPU connects to I/O or memory device via northbridge and southbridge
#+END_VERSE

#+BEGIN_SRC 
movq A, %rax
#+END_SRC
For the above operation, the following steps are done to read data from memory to register
1. CPU places address A on system bus
2. I/O bridge passes the signal along to memory bus
3. main memory read address from memory bus, fetches data from DRAM, write the data to the memory bus
4. I/O bridge translate data on memory bus to system bus
5. CPU read data from bus


#+BEGIN_SRC 
movq %rax, A
#+END_SRC
- CPU write address to bus
- CPU write data to bus
...
read disk is 10^5 slower than ready from DRAM, 10^9 slower than ready from SRAM.

**** Disk Geometry

[[file:img/disk-geometry.png]]

- Disk is consists of platters, called *disk drive*
- platter consists of two sides, or surfaces, coated with magnetic recording material
- rotating *spindle* spins at a fixed *rotation rate* (5400, or 15000 RPM)
- each surface consists of collection of concentric rings called *tracks*
- track is partitioned into a collection of *sectors*
- sectors are separated by *gaps*
- gaps store formatting bits that identify sectors
- cylinders: multiple-platter drives (includes track of all the surfaces)

**** Disk Capacity

Disk capacity is determined by the following factors
- recording density (bits/in) :: bits per inch segment of a track
- track density (tracks/in) :: tracks per 1 inch segment of the radius extending from the center of the platter
- areal density (bits/in^2) :: recording-density * track-density

Note that secters of tracks are same. The sectors on the innermost
track are much tighter than the ones on the outermost track. (larger gap)

$
Capacity = \frac{#byes}{sector} 
           \time \frac{average # sectors}{track} 
           \time \frac{#tracks}{surface} 
           \time \frac{#surfaces}{platter} 
           \time \frac{#platters}{disk}
$

***** Practice Problem 6.2 

Capacity = 15,000 * 6 * 500 * 1024 ~= 45GB

**** Disk Operation

Disk read and write bits stored on the magnetic surface using a
*read/write head* connected to the end of an *actuator arm*

[[file:img/disk-read-write.png]]

Disks with multiple platters have a separate read/write head for each
surface.

Access time for a sector have 3 main components
- seek time :: move arm to target sector, usually 3-9ms, maximum 20ms
- rational latency :: go up to the first bit
- transfer time :: time for read all bits of sector (per track)


***** Practic Problem 6.3 

$ T_{avg-rotation} = 0.5*60/12000*1000 = 2.5ms $

$ T_{avg-transfer} = 60/12000/300*1000 = 0.0167ms $

$ T_{access} = 2.5ms + 0.0167ms + 5ms = 7.5167ms $

**** Logical Disk Blocks

In disk, *disk controller* maps the physical sectors to logical blocks.

OS send command to disk controller to read/write from a particular
logical block (index N). The disk controller translate the request into
(surface, rtack, sector) triple and make the action.

#+BEGIN_VERSE
Disk can be used to store data after disk controller format it
1. filling in gaps betwee nsectors
2. take defected cylinders out of action
3. set spare cylinder for fallback usage
#+END_VERSE

**** Connecting I/O Devices

Unlike the system bus and memory bus, I/O buses are designed to be independent to the underlying CPU

[[file:img/io-bus.png]]

For example
- USB controller: 625 MB/s bandwidth for USB 3.0, 1,250 MB/s bandwidth for USB 3.1
- graphic card (adapter)
- host bus adapter: SCSI (can support multiple disk drives), SATA
- network adapter

**** Accessing Disks

CPU access I/O devices by visiting certain address space, which is
mapping to I/O device. Called *memory-mapped I/O*.

Example
1. CPU write read command to 0xa0 (mapped to disk controller)
2. CPU write logical block number to read
3. CPU write the main memory address the disk controller to write back
4. disk drive write back content to memory (DMA: direct memory access)
5. disk drive send interrupt to CPU
6. CPU stop currently job and read the content from main memory
...
features:
- reading is much faster than writeing
- write/read in units of page
- page can be written only after the entire block has been erased
- a block wears out after 100,000 repeated writes


why random writes are slower
1. erase a block take a relatively long time
2. block must be copied to another block before changing
...
disadvantages
- wear out after repated writes (flash translation layer attempts to maximize the lifetime of writing)
- 30 times expensive per byte than rotating disks


**** Practice Problem 6.5

- worst case for sequential writes: 8.65 years
- worse case for random writes: 13.4 years
- average cases: 17534.25 years
...
- speed: SRAM > DRAM > disk
- prices: SRAM > DRAM > SSD > disk
...
Modern computers make heavy use of SRAM-based caches to try to bridge
the processor-memory gap.

#+BEGIN_VERSE
Critical events in history change the industry and the workd
- 1950s Fortran
- 1960s IBM 360
- 1970s ARPANET
- 1980s IBM PC
...
Early in 21st centry, manufacturers could no longer increase CPU clock frequencies because it consume too much power.
The solution is to introduce multiple smaller processor cores.

Computers will get faster not because the clock frequency increases but because the number of cores in each processor increases,
and because architectural innovations increase the efficiency of programs running on those cores.
#+END_VERSE

** DONE Locality

*the principle of locality* has two aspects:
- temporal locality :: memory location is referenced once is likely to
     be referenced again multiple times in the near future
- spatial locality :: if a memory location is referenced once, then
     the program is likely to reference a nearby memory location in
     the near feature.

In general, programs with good locality run faster than programs with
poor locality.

Examples
- cache memory to hold recently referenced instructions and data items
- OS use main memory as a cache of the most recently referenced chunks
  of the virtual address space
- OS use main memory to cache disk blocks
- Web browser cache recently referenced documents on a local disk
- Web server cache requested document in front-end disk caches
...
- =sum= has no spatial locality, has good temporal locality
- =v= has good spatial locality, but poor temporal locality (read
  once). Read mode is also named *stride-1 reference pattern* or
  *sequential reference pattern*.


C arrays are laid out in memory row-wise. The access order affects the
locality.
#+BEGIN_SRC c
  // good spatial localiity, with stride-1/row-major reference pattern
  for (i=0; i<M; i++) {
    for (j=0; j<N; j++) {
      sum += a[i][j];
    }
  ...
  // poor spatial localiity, with stride-N reference pattern
  for (j=0; j<N; j++) {
    for (i=0; i<M; i++) {
      sum += a[i][j];
    }
...
Instructions are stored in memory, and are rarely overwritten
- =for= loop executes in sequential memory order, is good spatial locality
- loop body is executed multiple times, it also enjoys good temporal locality

*** Summary of Locality

Summary
1. program repeatedly reference the same variables enjoy good temporal locality
2. for stride-k reference patterns, the smaller the stride, the better
   the spatial locality.
3. loops have good temporal and spatial locality with respect to
   instruction fetches. The smaller the loop body and the greater the
   number of loop iterations, the better the locality.

**** DONE Practice Problem 6.8 

Ranks of spatial locality: b > c > d
