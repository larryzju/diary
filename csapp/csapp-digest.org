...
/linear address space/: the address space that the addresses are
consecutive. Represent with the number of bits (32-bit or 64-bit) that
are needed to represent the largest address:
...
Determine 
- if a *virtual page* is cached
- which *physical page* it is cached in
- Or is stored on disk
...
Swapped out VPE now points to a block located on disk.
...
/thrashing/: where pages are swapped in and out continuously.


In Linux, we can use =getrusage= API to check the page faults. See
...
*** TODO Integrating Caches and VM
*** TODO Speeding Up Address Translation with a TLB
*** TODO Multi-Level Page Tables
*** TODO Putting It Together: End-to-End Address Translation
