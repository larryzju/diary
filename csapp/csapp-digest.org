...
bytes. The bytes is indexed by *address*. In the underlying the
virtual memroy is made up by RAM, disk storage, special hardware, OS
that provides the program with what appears to be a monolithic byte
array.
...
Acutally, C compiler maintains pointer's type information, the
machine-level program it generates has no information about data
types.

Pointer are the central feature of C:
1. reference elements of data structure
2. combine value and type
